...
class PR {
	...
	static function LoadProgs():Void {
		...
		PR.netnames = PR.NewString('', (untyped SV).svs.maxclients << 5);
		...
		while (true) {
			...
			switch (st.op) {
				...
				case PROp.storep_f | PROp.storep_ent | PROp.storep_fld | PROp.storep_s | PROp.storep_fnc:
					...
					(untyped SV).server.edicts[Math.floor(ptr / PR.edict_size)].v_int[((ptr % PR.edict_size) - 96) >> 2] = PR.globals_int[st.a];
				case PROp.storep_v:
					var ed:Edict = (untyped SV).server.edicts[Math.floor(PR.globals_int[st.b] / PR.edict_size)];
				...
				case PROp.address:
					...
					if ((ed == 0) && ((untyped SV).server.loading != true))
				...
				case PROp.load_f | PROp.load_fld | PROp.load_ent | PROp.load_s | PROp.load_fnc:
					PR.globals_int[st.c] = (untyped SV).server.edicts[PR.globals_int[st.a]].v_int[PR.globals_int[st.b]];
				case PROp.load_v:
					var ed:Edict = (untyped SV).server.edicts[PR.globals_int[st.a]];
					var ptr = PR.globals_int[st.b];
				...
				case PROp.done | PROp.ret:
					...
					if (PR.depth == exitdepth)
						return;
				case PROp.state:
					var ed:Edict = (untyped SV).server.edicts[PR.globals_int[PR.globalvars.self]];
